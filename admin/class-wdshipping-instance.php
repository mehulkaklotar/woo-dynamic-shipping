...
if( ! class_exists( 'WDShipping_Instance' ) ) {
	...
	class WDShipping_Instance extends WC_Shipping_Method {
		 ...
		 * @param $id
		 * @param $method_title
		 * @param $flat_rate_option
		 * @param $description
		 */
		function __construct( $id, $method_title, $flat_rate_option, $description ) {
			$this->id                 = $id;
			$this->method_title       = __( $method_title, 'woocommerce' );
			$this->flat_rate_option   = 'woocommerce_' . $flat_rate_option;
			...
			if ( sizeof( $this->options ) > 0 ) {
				...
				foreach ( $this->options as $option ) {
					...
					if ( is_array( $extra_rate['cost'] ) ) {
						$extra_rate['cost']['order'] = $extra_rate['cost']['order'] + $this_cost;
				...
				<td class="forminp" id="<?php echo $this->id; ?>_flat_rates">
					<table class="shippingrows widefat" cellspacing="0">
						...
						<tr>
							...
							<th><?php _e( 'Cost', 'woocommerce' ); ?> <a class="tips"
							                                             data-tip="<?php _e( 'Cost, excluding tax.', 'woocommerce' ); ?>">[?]</a>
							</th>
							<th><?php _e( 'Handling Fee', 'woocommerce' ); ?> <a class="tips"
							                                                     data-tip="<?php _e( 'Fee excluding tax. Enter an amount, e.g. 2.50, or a percentage, e.g. 5%.', 'woocommerce' ); ?>">[?]</a>
							</th>
						</tr>
						</thead>
						...
						<tr>
							...
							<td><input type="text"
							           value="<?php echo esc_attr( wc_format_localized_price( $this->cost ) ); ?>"
							           name="default_cost" placeholder="<?php _e( 'N/A', 'woocommerce' ); ?>" size="4"
							           class="wc_input_price"/></td>
							<td><input type="text"
							           value="<?php echo esc_attr( wc_format_localized_price( $this->fee ) ); ?>"
							           name="default_fee" placeholder="<?php _e( 'N/A', 'woocommerce' ); ?>" size="4"
							           class="wc_input_price"/></td>
						</tr>
						<?php
						$i = - 1;
						...
						<tfoot>
						<tr>
							<th colspan="4"><a href="#" class="add button"><?php _e( 'Add Cost', 'woocommerce' ); ?></a>
								<a href="#"
								   class="remove button"><?php _e( 'Delete selected costs', 'woocommerce' ); ?></a></th>
						</tr>
						</tfoot>
					</table>
		...
		function process_flat_rates() {
			...
			if ( isset( $_POST[ $this->id . '_class' ] ) ) {
				$flat_rate_class = array_map( 'wc_clean', $_POST[ $this->id . '_class' ] );
			}
			if ( isset( $_POST[ $this->id . '_cost' ] ) ) {
				$flat_rate_cost = array_map( 'stripslashes', $_POST[ $this->id . '_cost' ] );
			}
			if ( isset( $_POST[ $this->id . '_fee' ] ) ) {
				$flat_rate_fee = array_map( 'stripslashes', $_POST[ $this->id . '_fee' ] );
			}
